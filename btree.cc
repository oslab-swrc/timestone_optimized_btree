...
TEST(tss, spatial_safety_master)
{
	ts::ts_persistent_ptr<list> p_list;
	char val[32] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ,./,/";
	...
	/* violate spatial safety for master obj*/
	try {
		ts::ts_txn::run_rw(ts::snapshot, [&]() {
			p_list->add_unsafe_master(25, val);
		});
	} catch (const ts_exception &exception) {
...
TEST(tss, spatial_safety_copy)
{
	list *p_list;
	char val[32] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ,./,/";

	ts::ts_system::init();
	ts::ts_txn::run_rw(ts::snapshot, [&]() { p_list = new (true) list; });

	/* violate spatial safety for copy obj*/
	try {
		ts::ts_txn::run_rw(ts::snapshot,
				   [&]() { p_list->add_unsafe_copy(25, val); });
	} catch (const ts_exception &exception) {
		std::cerr << exception.what() << std::endl;
		exit(0);
	}
...
TEST(tss, out_of_bound_access)
{
	...
	ts::ts_txn::run_ro(ts::snapshot, [&]() { array.print(); });
	/* out of bound access*/
	std::cout << "reading an out of bound index" << std::endl;
	try {
		ts::ts_txn::run_ro(ts::snapshot,
				   [&]() { array.read_at_unsafe(10); });
	} catch (const ts_exception &exception) {
		std::cerr << exception.what() << std::endl;
		exit(0);
	}
...
TEST(tss, temporal_safety_master)
{
	list *p_list;
	void *ret;
	ts_obj_hdr_t *ohdr;
	...
	std::cout << "============= insert key 1 ============== \n"
		  << std::endl;
	ts::ts_txn::run_rw(ts::snapshot, [&]() { ret = p_list->insert(1); });
	ohdr = obj_to_obj_hdr(ret);
	std::cout << "new_node= " << ret << "\t"
		  << "new_node tag= " << ohdr->ptr_tag << std::endl;
	std::cout << "=========================== \n" << std::endl;

	std::cout << "=============  print list ============== \n" << std::endl;
	ts::ts_txn::run_rw(ts::snapshot, [&]() { p_list->print(); });
	std::cout << "=========================== \n" << std::endl;

	std::cout << "============= buggy delete key 1 ============== \n"
		  << std::endl;
	ts::ts_txn::run_rw(ts::snapshot,
			   [&]() { ret = p_list->remove_buggy(1); });
	ohdr = obj_to_obj_hdr(ret);
	std::cout << "del_node= " << ret << "\t"
		  << "del_node tag= " << ohdr->ptr_tag << std::endl;
	std::cout << "=========================== \n" << std::endl;

	std::cout << "============= read key 1 ============== \n" << std::endl;
	ts::ts_txn::run_rw(ts::snapshot,
			   [&]() { ASSERT_EQ(p_list->contains(1), 1); });
	std::cout << "=========================== \n" << std::endl;
	ts::ts_system::finish();
	ts::ts_system::print_stats();
}

TEST(tss, temporal_safety_copy)
{
	list *p_list;
	void *ret;
	ts_obj_hdr_t *ohdr;
	...
	ts::ts_txn::run_rw(ts::snapshot, [&]() { ret = p_list->insert(3); });
	ohdr = obj_to_obj_hdr(ret);
	std::cout << "new_node= " << ret << "\t"
		  << "new_node tag= " << ohdr->ptr_tag << std::endl;

	ts::ts_txn::run_rw(ts::snapshot, [&]() { ret = p_list->insert(2); });
	ohdr = obj_to_obj_hdr(ret);
	std::cout << "new_node= " << ret << "\t"
		  << "new_node tag= " << ohdr->ptr_tag << std::endl;
